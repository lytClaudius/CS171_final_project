...
std::string Shader::getCodeFromFiles(const std::vector<std::string> &paths)
{
    std::string fullCode = "#version 430 core\n"; // 确保版本号在最前面
    for (const auto &path : paths)
    {
        std::string partCode = getCodeFromFile(path);
        // 如果子文件里也有 #version，建议去掉它，或者确保子文件里不写版本号
        // 简单处理：直接追加
        fullCode += "// File: " + path + "\n"; // 方便调试
        fullCode += partCode + "\n";
    }
...
Shader::Shader(const std::string &vsPath, const std::vector<std::string> &fsPaths)
{
    std::string vsCode = getCodeFromFile(vsPath);
    std::string fsCode = getCodeFromFiles(fsPaths); // 拼接所有的 FS 文件
    initWithCode(vsCode, fsCode);
}
...
void Shader::setBool(const std::string &name, GLboolean value) const { glUniform1i(getUniformLocation(name), (int)value); }
